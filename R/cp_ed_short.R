...
#' use in profiles.  This Education Chart shows fewer educational categories.
#'
...
cp_ed_short=function(fips, state="08", fips2="", state2="08", base=12){
  ...
  d13pm=d13p%>%
    mutate(ed1=b15003002+b15003003+b15003004+b15003005+b15003006+b15003007+b15003008+b15003009+b15003010+b15003011+
           ...
           ed4=b15003022+b15003023+b15003024+b15003025)%>%
    select(geoname:geonum,ed1:ed4)%>%
  ...
  p=ggplot(d, aes(x=agecat, y=p, fill=geoname))+
    ...
    geom_text(aes(x=agecat, ymax=p, label=percent(p), vjust=1.4),position=position_dodge(width=.9), family="sans", size=base*0.4166667, color="white")+

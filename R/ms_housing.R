...
housing_10=inner_join(h3_10,h5_10)%>%inner_join(h4_10)%>%
  gather(var, Census.2010, Total:Renter, -geoname, -state, -county, -place,-tract,-bg,-geonum)
...
housing_00=inner_join(h3_00,h5_00)%>%inner_join(h4_00)%>%
  gather(var, Census.2000, Total:Renter, -geoname, -state, -county, -place,-tract,-bg,-geonum)
